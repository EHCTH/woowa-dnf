...
public class NeopleClient {
    ...
    public <T> T get(String path, Class<T> type, Object... vars) {
         return handler.handle(() -> neopleRestClient.get()
                .uri(u -> u.path(path).queryParam(API_KEY, props.apiKey()).build(vars))
                .retrieve()
                .body(type));
    }

    public <T> T get(String path, Class<T> type, Map<String, ?> query, Object... vars) {
        return handler.handle(() -> neopleRestClient.get()
                .uri(uri -> {
